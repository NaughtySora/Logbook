...
class LogBook {
  ...
  constructor({ dir, rotation = Infinity } = {}) {
  ...
  #resume(channel) {
    ...
    for (let i = 0; i < copy.length; i++) {
      process.nextTick(() => void this.#write(copy[i]));
    }
  ...
  #write(logs, channel) {
    ...
    try {
      if (!stream.write(this.#stringify(logs), "utf-8")) {
  ...
  cursor(channel, { from = 0, to = Infinity, encoding = "utf8" } = {}) {
    ...
    const files = this.#readDir(dir, { from, to });
    const self = this;
    return (async function* gen() {
      if (self.#closed) return;
      for await (const file of files) {
        yield fs.readFile(path.resolve(dir, file), { encoding });
      }
    })();
  }
